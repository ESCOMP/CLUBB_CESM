...
!+++ARH
    ...
    real( kind = core_rknd ), intent(inout), dimension(gr%nz) :: &
      ...
      mf_thlflx,  mf_qtflx,     & ! total flux of theta_l [K m/s], q_t [(kg/kg) m/s]
...
! <--- h1g, 2012-06-14
    ...
    rtp3_zm  = zt2zm( rtp3 )
    up2_zt   = max( zm2zt( up2 ), w_tol_sqd ) ! Positive definite quantity
    up3_zm   = zt2zm( up3 )
    vp2_zt   = max( zm2zt( vp2 ), w_tol_sqd ) ! Positive definite quantity
    vp3_zm   = zt2zm( vp3 )

    do i = 1, sclr_dim, 1
       sclrp2_zt(:,i) = max( zm2zt( sclrp2(:,i) ), sclr_tol(i)**2 ) ! Pos. def. quantity
       sclrp3_zm(:,i)  = zt2zm( sclrp3(:,i) )
    ...
    Sku_zt(1:gr%nz) = Skx_func( up2_zt(1:gr%nz), up3(1:gr%nz), w_tol )
    Sku_zm(1:gr%nz) = Skx_func( up2(1:gr%nz), up3_zm(1:gr%nz), w_tol )

    Skv_zt(1:gr%nz) = Skx_func( vp2_zt(1:gr%nz), vp3(1:gr%nz), w_tol )
    Skv_zm(1:gr%nz) = Skx_func( vp2(1:gr%nz), vp3_zm(1:gr%nz), w_tol )

    do i = 1, sclr_dim, 1
       Sksclr_zt(1:gr%nz,i) &
       = Skx_func( sclrp2_zt(1:gr%nz,i), sclrp3(1:gr%nz,i), sclr_tol(i) )
       Sksclr_zm(1:gr%nz,i) &
       = Skx_func( sclrp2(1:gr%nz,i), sclrp3_zm(1:gr%nz,i), sclr_tol(i) )
    enddo ! i = 1, sclr_dim, 1
...
!  dry_a,moist_a,dry_w,moist_w, ... kts:kte+1
     ...
     real(kind=core_rknd), dimension(nz), intent(in) :: u,v,th,thl,qt,qc,thv, & ! all on thermodynamic/midpoint levels
                                                        zt,dzt,iexner,p
     real(kind=core_rknd), dimension(nz), intent(in) :: thl_zm,qt_zm ! theta_l and q_t


     real(kind=core_rknd), intent(in) :: ust, wthl, wqt
     real(kind=core_rknd), intent(inout) :: pblh ! MKW TODO: figure out where ust and pblh come - surface_varnce and ???
     real(kind=core_rknd),value :: dt ! only needed for random number generator
...
! updraft properties
     real(kind=core_rknd), dimension(nz,nup) ::                &
...
!           print*,'k,ztu,ztd=',k,zt(k),zt(k-1)
       ...
       do i=1,nup
         do k=2,nz
           ent(k,i) = real( enti(k,i))*ent0/dzt(k) ! MKW TODO: also input invrs_dzt? only used here.
         enddo
       enddo

       ! surface conditions
!          wstar  = max( wstarmin, (gravit/thv(1)*wthv*pblh)**(1./3.) )
       wstar  = max( wstarmin, (gravit/thv(1)*wthv*pblh)**(1./3.) ) ! MKW NOTE: is it better to use kts instead of index 1 for surface? we'll find out
       qstar  = wqt / wstar
       thstar = wthl / wstar
...
!       print *,'max(upa)=',maxval(upa)
       ...
       do i=1,nup
         do k=2,nz!kts+1,kte+1
           ...
           entexpu = exp(-ent(k,i)*dzt(k)/3.)  !orig
!              entexpu = exp(-ent(k,i)*(zt(k)-zt(k-1))/1.)
...
!                                     thvn, qcn, thn, qln, qin)
           ...
           wp = wb*ent(k,i)
           if (wp==0.) then
             wn2 = upw(k-1,i)**2+2.*wa*b*dzt(k-1)
           else
...
!     ..
!     .. Local Scalars ..
	REAL(kind=core_rknd)          :: b1, b2, c, c0, c1, c2, c3, del, difmuk, e, fk, fx, fy, g,  &
                    omega, px, py, t, u, v, x, xx
	REAL(kind=core_rknd), SAVE    :: s, d, p, q, p0
        INTEGER       :: j, k, kflag
	LOGICAL, SAVE :: full_init
        INTEGER, SAVE :: l, m
!     ..
!     .. Local Arrays ..
	REAL(kind=core_rknd), SAVE    :: pp(35)
!     ..
!     .. Data statements ..
	REAL(kind=core_rknd), PARAMETER :: a0 = -.5, a1 = .3333333, a2 = -.2500068, a3 = .2000118,  &
                a4 = -.1661269, a5 = .1421878, a6 = -.1384794,   &
                 a7 = .1250060

	REAL(kind=core_rknd), PARAMETER :: fact(10) = (/ 1., 1., 2., 6., 24., 120., 720., 5040.,  &
            40320., 362880. /)
...
!     STEP N. NORMAL SAMPLE - random_normal() FOR STANDARD NORMAL DEVIATE
  ...
  use clubb_precision, only: &
	...
	!     Local variables
	REAL(kind=core_rknd)     :: s = 0.449871, t = -0.386595, a = 0.19600, b = 0.25472,           &
